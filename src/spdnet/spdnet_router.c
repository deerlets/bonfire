...
{
    return spdnet_find_routing_item(
...
spdnet_router_sendmsg(struct spdnet_node *snode, struct spdnet_msg *msg)
{
    ...
    rc = zmq_send(
        socket,
        dst_routing->nexthop_id,
        dst_routing->nexthop_len,
        ZMQ_SNDMORE);
    assert(rc != -1);
    ...
    if (dst_routing->nexthop_type == SPDNET_ROUTER) {
        ...
        // remove self-rid
        zmq_msg_t dstid_msg;
        size_t dstid_msg_size =
            (char *)MSG_DSTID_DATA(msg) + MSG_DSTID_SIZE(msg) - dstid;
                                   ...
                                   void *id, size_t *len)
{
    ...
    INIT_SPDNET_ROUTING_ITEM(
        item, remote_id, remote_len, remote_id, remote_len, SPDNET_ROUTER);
                                     ...
                                     void *id, size_t len)
{
    ...
    if (!item) {
        ...
        INIT_SPDNET_ROUTING_ITEM(
            item, gw, strlen(gw), id, len, SPDNET_ROUTER);
        list_add(&item->node, &router->routing_table);
    } else
        INIT_SPDNET_ROUTING_ITEM(
            item, gw, strlen(gw), id, len, SPDNET_ROUTER);
